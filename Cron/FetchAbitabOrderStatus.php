...
use MercadoPago\PaymentMagento\Gateway\Config\Abitab;
use MercadoPago\PaymentMagento\Model\Console\Command\Notification\FetchStatus;

/**
 * CronTab for fetch Abitab Order Status.
 */
class FetchAbitabOrderStatus
{
    /**
     * @var Logger
     */
    ...
    public function execute()
    {
        ...
        $orders->getSelect()
                ...
                ->where('sop.method = ?', Abitab::METHOD);

        foreach ($orders as $order) {
            $orderId = $order->getEntityId();

            $this->logger->debug([
                'fetch'   => 'Fetch Status Abitab for Order Id '.$orderId,
            ]);
