...
/**
 * CronTab for fetch Banamex Order Status.
 */
class FetchBanamexOrderStatus
{
    /**
     * @var Logger
     */
    ...
    public function execute()
    {
        ...
        $orders->getSelect()
                ...
                ->where('sop.method = ?', ConfigBanamex::METHOD);

        foreach ($orders as $order) {
            $orderId = $order->getEntityId();

            $this->logger->debug([
                'fetch'   => 'Fetch Status Banamex for Order Id '.$orderId,
            ]);
