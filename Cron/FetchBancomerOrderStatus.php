...
/**
 * CronTab for fetch Bancomer Order Status.
 */
class FetchBancomerOrderStatus
{
    /**
     * @var Logger
     */
    ...
    public function execute()
    {
        ...
        $orders->getSelect()
                ...
                ->where('sop.method = ?', ConfigBancomer::METHOD);

        foreach ($orders as $order) {
            $orderId = $order->getEntityId();

            $this->logger->debug([
                'fetch'   => 'Fetch Status Bancomer for Order Id '.$orderId,
            ]);
