...
/**
 * CronTab for fetch Pago Efectivo Order Status.
 */
class FetchPagoEfectivoOrderStatus
{
    /**
     * @var Logger
     */
    ...
    public function execute()
    {
        ...
        $orders->getSelect()
                ...
                ->where('sop.method = ?', ConfigPagoEfectivo::METHOD);

        foreach ($orders as $order) {
            $orderId = $order->getEntityId();

            $this->logger->debug([
                'fetch'   => 'Fetch Status PagoEfectivo for Order Id '.$orderId,
            ]);
