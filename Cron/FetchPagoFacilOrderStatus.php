...
/**
 * CronTab for fetch Pago Facil Order Status.
 */
class FetchPagoFacilOrderStatus
{
    /**
     * @var Logger
     */
    ...
    public function execute()
    {
        ...
        $orders->getSelect()
                ...
                ->where('sop.method = ?', ConfigPagoFacil::METHOD);

        foreach ($orders as $order) {
            $orderId = $order->getEntityId();

            $this->logger->debug([
                'fetch'   => 'Fetch Status PagoFacil for Order Id '.$orderId,
            ]);
