...
/**
 * CronTab for fetch Pay Cash Order Status.
 */
class FetchPayCashOrderStatus
{
    /**
     * @var Logger
     */
    ...
    public function execute()
    {
        ...
        $orders->getSelect()
                ...
                ->where('sop.method = ?', ConfigPayCash::METHOD);

        foreach ($orders as $order) {
            $orderId = $order->getEntityId();

            $this->logger->debug([
                'fetch'   => 'Fetch Status PayCash for Order Id '.$orderId,
            ]);
