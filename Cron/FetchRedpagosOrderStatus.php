...
/**
 * CronTab for fetch Redpagos Order Status.
 */
class FetchRedpagosOrderStatus
{
    /**
     * @var Logger
     */
    ...
    public function execute()
    {
        ...
        $orders->getSelect()
                ...
                ->where('sop.method = ?', ConfigRedpagos::METHOD);

        foreach ($orders as $order) {
            $orderId = $order->getEntityId();

            $this->logger->debug([
                'fetch'   => 'Fetch Status Redpagos for Order Id '.$orderId,
            ]);
